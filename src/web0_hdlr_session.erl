...
%% 
handle_cookie_new(_, undefined, R, S) -> web0_hdlr_common:'400'(R, [msg_this_node(), "session data missing"], S);
handle_cookie_new(NewSid, Data, R0, S) -> 
    ...
    web0_hdlr_common:'200'(R1, msg_new_session(NewSid, NewData), S).
...
%%
handle_cookie_show(undefined, R, S) -> web0_hdlr_common:'200'(R, "no session id provided", S);
handle_cookie_show(OldSid, R, S) ->    
    Data = mnesia:dirty_read({session, OldSid}),
    handle_cookie_show(OldSid, Data, R, S).
handle_cookie_show(Sid, [], R, S) -> web0_hdlr_common:'200'(R, ["session id ", Sid, " does not exist"], S);
handle_cookie_show(Sid, [{session, _, D}], R, S) -> web0_hdlr_common:'200'(R, msg_old_session(Sid, D), S).
